...
// tools/ai-oracle.ts
var ai_oracle_exports = {};
__export(ai_oracle_exports, {
  POST: () => POST,
  mcp: () => mcp,
  payment: () => payment,
  schema: () => schema
});
module.exports = __toCommonJS(ai_oracle_exports);
...
// node_modules/opentool/dist/payment/index.js
var PAYMENT_SCHEMA_VERSION = 1;
var paymentSchemaVersionSchema = external_exports.literal(PAYMENT_SCHEMA_VERSION);
var decimalStringSchema = external_exports.string().regex(/^(?:0|[1-9]\d*)(?:\.\d+)?$/, "Value must be a positive decimal string");
var currencySchema = external_exports.object({
  code: external_exports.string().min(2).max(12).transform((value) => value.toUpperCase()),
  symbol: external_exports.string().min(1).max(6).optional(),
  decimals: external_exports.number().int().min(0).max(36).optional(),
  kind: external_exports.enum(["fiat", "crypto"]).default("crypto").optional(),
  description: external_exports.string().optional()
});
var paymentAmountSchema = external_exports.object({
  value: decimalStringSchema,
  currency: currencySchema,
  display: external_exports.string().optional()
});
var cryptoAssetSchema = external_exports.object({
  symbol: external_exports.string().min(2).max(12),
  network: external_exports.string().min(1).optional(),
  chainId: external_exports.number().int().min(0).optional(),
  address: external_exports.string().min(1).optional(),
  decimals: external_exports.number().int().min(0).max(36).optional(),
  standard: external_exports.enum(["erc20", "spl", "custom"]).default("erc20").optional(),
  description: external_exports.string().optional()
});
var facilitatorConfigSchema = external_exports.object({
  url: external_exports.string().url(),
  vendor: external_exports.string().optional(),
  verifyPath: external_exports.string().default("/verify").optional(),
  settlePath: external_exports.string().default("/settle").optional(),
  apiKey: external_exports.string().optional(),
  apiKeyEnv: external_exports.string().optional(),
  apiKeyHeader: external_exports.string().default("Authorization").optional(),
...
var settlementTermsSchema = external_exports.object({
  windowSeconds: external_exports.number().int().positive().optional(),
  targetConfirmations: external_exports.number().int().positive().optional(),
  finalityDescription: external_exports.string().optional(),
  slaDescription: external_exports.string().optional()
});
var paymentFieldSchema = external_exports.object({
  key: external_exports.string().min(1),
  label: external_exports.string().min(1),
  required: external_exports.boolean().default(true).optional(),
  description: external_exports.string().optional(),
  example: external_exports.string().optional()
});
var x402ProofSchema = external_exports.object({
  mode: external_exports.literal("x402"),
  scheme: external_exports.string().min(1),
  network: external_exports.string().min(1),
  version: external_exports.number().int().min(1).optional(),
  facilitator: facilitatorConfigSchema.optional(),
  verifier: external_exports.string().optional()
});
var directProofSchema = external_exports.object({
  mode: external_exports.literal("direct"),
  proofTypes: external_exports.array(external_exports.string().min(1)).nonempty(),
  verifier: external_exports.string().optional(),
  instructions: external_exports.string().optional(),
  fields: external_exports.array(paymentFieldSchema).optional(),
  allowsManualReview: external_exports.boolean().optional()
});
var paymentProofSchema = external_exports.discriminatedUnion("mode", [
  x402ProofSchema,
  directProofSchema
]);
var paymentOptionSchema = external_exports.object({
  ...
  resource: external_exports.string().url().optional(),
...
var schema = external_exports.object({
  question: external_exports.string().min(1).default("What is the price of BTC?").describe("Question to answer")
});
var payment = definePayment({
  amount: "0.001",
  currency: "USDC",
